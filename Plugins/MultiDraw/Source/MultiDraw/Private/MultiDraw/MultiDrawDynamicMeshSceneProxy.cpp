...
FMultiDrawDynamicMeshSceneProxy::FMultiDrawDynamicMeshSceneProxy(UMultiDrawDynamicMeshComponent* Component)
...
void FMultiDrawDynamicMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views,
	const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	...
	auto PerPassDrawSettingses=ParentComponent->PerPassDrawSettingses;
		...
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			for (FMeshRenderBufferSet* BufferSet : Buffers)
			{
				...
				for (const FMultiDrawSettings& PerPassDrawSetting : PerPassDrawSettingses)
				{
					...
					if (View->IsPerspectiveProjection() || !(AllowDebugViewmodes() && ViewFamily.EngineShowFlags.Wireframe))
					{
						DrawMultiDrawBatch(Collector, *BufferSet, BufferSet->IndexBuffer, CurrentMultiDrawMaterial->GetRenderProxy(), false, DepthPriority, ViewIndex, DynamicPrimitiveUniformBuffer,PerPassDrawSetting.bCastShadow,PerPassDrawSetting.CullingMode);
					}
				}
			}
		}
...
void FMultiDrawDynamicMeshSceneProxy::DrawBatch(FMeshElementCollector& Collector,
...
void FMultiDrawDynamicMeshSceneProxy::DrawMultiDrawBatch(FMeshElementCollector& Collector,
	const FMeshRenderBufferSet& RenderBuffers, const FDynamicMeshIndexBuffer32& IndexBuffer,
	FMaterialRenderProxy* UseMaterial, bool bWireframe, ESceneDepthPriorityGroup DepthPriority, int ViewIndex,
	FDynamicPrimitiveUniformBuffer& DynamicPrimitiveUniformBuffer,bool bCastShadow,EMultiDrawCullingMode CullingMode) const
{
	FMeshBatch& Mesh = Collector.AllocateMesh();
	FMeshBatchElement& BatchElement = Mesh.Elements[0];
	BatchElement.IndexBuffer = &IndexBuffer;
