...
#define LOCTEXT_NAMESPACE "MultiDrawStaticMeshSceneProxy"

FMultiDrawStaticMeshSceneProxy::FMultiDrawStaticMeshSceneProxy(UMultiDrawStaticMeshComponent* Component, bool bForceLODsShareStaticLighting) :
	FStaticMeshSceneProxy(dynamic_cast<UStaticMeshComponent*>(Component)
	,bForceLODsShareStaticLighting)
...
void FMultiDrawStaticMeshSceneProxy::DrawStaticElements(FStaticPrimitiveDrawInterface* PDI)
{
	...
	if (!HasViewDependentDPG())
	{
		...
		const UMultiDrawStaticMeshComponent* MultiDrawStaticMeshComponent = dynamic_cast<const UMultiDrawStaticMeshComponent *>(ComponentPtr);
		auto PerPassDrawSettingses=MultiDrawStaticMeshComponent->PerPassDrawSettingses;
					...
					if (GetMeshElement(LODIndex, BatchIndex, SectionIndex, PrimitiveDPG, bIsMeshElementSelected, true, BaseMeshBatch))
					{
						if (NumRuntimeVirtualTextureTypes > 0)
						{
							...
							for (ERuntimeVirtualTextureMaterialType MaterialType : RuntimeVirtualTextureMaterialTypes)
							{
								...
								{
									if(PerPassDrawSetting.MaterialInterface == nullptr || PerPassDrawSetting.bDraw==false)
										continue;
        
									MeshBatch.MaterialRenderProxy = PerPassDrawSetting.MaterialInterface->GetRenderProxy();
						...
						}
						{
							...
							for (const FMultiDrawSettings& PerPassDrawSetting : PerPassDrawSettingses)
							{
								if(PerPassDrawSetting.MaterialInterface == nullptr || PerPassDrawSetting.bDraw==false || PerPassDrawSetting.ElementIndex != SectionIndex)
									continue;
        
								BaseMeshBatch.MaterialRenderProxy = PerPassDrawSetting.MaterialInterface->GetRenderProxy();
