	...
	else
	{
		...
		if (LODSections.Num() > 0 && LODIndex >= FirstLODIdx)
		{
			...
			for (FSkeletalMeshSectionIter Iter(LODIndex, *MeshObject, LODData, LODSection); Iter; ++Iter)
			{
				...
				{
					if(PerPassSetting.MaterialInterface == nullptr || PerPassSetting.bDraw==false)
						continue;
...
void FMultiDrawSkeletalMeshSceneProxy::UpdateLooseParametersUniformBuffer(const FSceneView* View, const int32 SectionIndex, const FMeshBatch& Mesh, const FGPUSkinBatchElementUserData* BatchUserData) const
{
