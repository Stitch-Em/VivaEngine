...
class FMultiDrawProceduralMeshSceneProxy final : public FPrimitiveSceneProxy
{
public:
	...
	FMultiDrawProceduralMeshSceneProxy(UMultiDrawProceduralMeshComponent* Component)
	...
	virtual ~FMultiDrawProceduralMeshSceneProxy()
	{
	...
	virtual void GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const override
	{
		...
		auto PerPassDrawSettingses=Component->PerPassDrawSettingses;
			...
			if (Section != nullptr && Section->bSectionVisible)
			{
				...
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
				{
					if (VisibilityMap & (1 << ViewIndex))
					{
						...
						//MultiDraw
						for (const FMultiDrawSettings& PerPassDrawSetting : PerPassDrawSettingses)
						{
							UMaterialInterface* CurrentMultiDrawMaterial=PerPassDrawSetting.MaterialInterface;
							const int32 MaterialIndex = Sections.IndexOfByKey(Section);
							if (CurrentMultiDrawMaterial == nullptr || !PerPassDrawSetting.bDraw || PerPassDrawSetting.ElementIndex != MaterialIndex)
								continue;
							
							FMeshBatch& MultiDrawMesh = Collector.AllocateMesh();
							FMeshBatchElement& OutlineMeshBatchElement = MultiDrawMesh.Elements[0];
							OutlineMeshBatchElement.IndexBuffer = &Section->IndexBuffer;
							MultiDrawMesh.bWireframe = bWireframe;
							MultiDrawMesh.VertexFactory = &Section->VertexFactory;
							MultiDrawMesh.MaterialRenderProxy = MaterialProxy;
							...
							MultiDrawMesh.ReverseCulling = IsLocalToWorldDeterminantNegative();
							MultiDrawMesh.Type = PT_TriangleList;
							MultiDrawMesh.DepthPriorityGroup = SDPG_World;
							MultiDrawMesh.bCanApplyViewModeOverrides = false;
							
							MultiDrawMesh.MaterialRenderProxy = CurrentMultiDrawMaterial->GetRenderProxy();
							MultiDrawMesh.CastShadow=PerPassDrawSetting.bCastShadow;
							switch (PerPassDrawSetting.CullingMode)
							{
							case EMultiDrawCullingMode::FrontfaceCulling:
								MultiDrawMesh.ReverseCulling = !MultiDrawMesh.ReverseCulling;
								break;
							case  EMultiDrawCullingMode::BackfaceCulling:
								break;
							case  EMultiDrawCullingMode::DoubleSide:
								MultiDrawMesh.bDisableBackfaceCulling=true;
								break;
							}
	    
							Collector.AddMesh(ViewIndex, MultiDrawMesh);
						}
					}
				}
			}
...
private:
	...
	UMultiDrawProceduralMeshComponent* Component;
};